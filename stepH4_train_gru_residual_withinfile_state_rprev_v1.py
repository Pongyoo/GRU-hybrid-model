# ============================================================
# stepH4_train_gru_residual_withinfile_state_rprev_v1.py  (v1) [PATCHED]
# Phase 4: Train GRU as Residual AR-Compensator (leakage-safe)
#
# PATCH (stride-consistent delta + add state r_prev):
#  - Target: dr_stride = r[t_end] - r[t_end - stride]
#  - Drop sequences where prev_idx < 0 (mask, no cheating)
#  - NO-AR input stays the same BUT add 1 state channel:
#      r_prev_state = residual_raw[t_end - stride]
#    broadcast across timesteps => X becomes 5 features
# ============================================================
...
def main():
    ...
    # --- make unique subfolder per npz (avoid overwrite) ---
    npz_base = os.path.splitext(os.path.basename(args.npz))[0]
    ...
    # --------------------------------------------------------
    # Target from residual_raw (NO residual history in input):
    # For each sequence i starting at s=i*stride:
    #   end index:  t_end = s + seq_len - 1
    #   prev index: t_prev = t_end - stride        ✅ stride-step
    #   target: dr = r[t_end] - r[t_prev]
    #
    # Clean method: drop sequences where t_prev < 0
    # --------------------------------------------------------
    ...
    # ----- PATCH START: stride-consistent delta target + r_prev_state -----
    end_idx  = idxs + (seq_len - 1)
    prev_idx = end_idx - stride
    ...
    y_end  = res_raw[end_idx]
    y_prev = res_raw[prev_idx]
    y = (y_end - y_prev).astype(np.float32)

    # =========================
    # NEW: add state feature (r_prev) as 1 channel
    # r_prev_state = residual_raw[t_end - stride] (same prev_idx used in target)
    # broadcast to full sequence length (constant within each sequence)
    # =========================
    r_prev = y_prev.astype(np.float32)  # residual_raw at t_prev
    r_prev_seq = np.repeat(r_prev[:, None, None], seq_len, axis=1)  # (N_seq, T, 1)

    # concat as 5th feature
    X = np.concatenate([X, r_prev_seq], axis=-1).astype(np.float32)

    # update N_seq after filtering
    N_seq = X.shape[0]
    D = X.shape[-1]  # now should be 5
    # ----- PATCH END -----

    print("\n=== Task definition ===")
    print("Input : [I_rms, f_inst, G_res, A_phys, r_prev_state] (NO AR history)")
    print("  where r_prev_state = residual_raw[t_end - stride] (1 value per sequence, broadcast in time)")
    print("Target: delta_residual_raw at sequence end (stride-step: r[t]-r[t-stride])")
    print(f"Final X dim = {D}")
    print(f"After valid-filter: N_seq={N_seq}")
