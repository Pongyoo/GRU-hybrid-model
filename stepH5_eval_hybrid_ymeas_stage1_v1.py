# ============================================================
# stepH5_eval_hybrid_ymeas_stage1_v1.py  [PATCHED stage1 + stride-consistent + one-step hybrid]
...
#  - ADD-STAGE FEATURE to match Step4 training (5D input):
#      stage = residual0[prev_idx] broadcast across seq_len and concat to X0
#
# NEW PATCH (เพิ่มอย่างเดียว ไม่แตะ free-run เดิม):
#  - Add "one-step hybrid" (teacher-forcing state):
#      r_hat_onestep = r_prev_true + dr_pred
#      y_hybrid_onestep = y_phys + r_hat_onestep
#  - Add metrics + prints + (optional) plots for one-step
# ============================================================
...
def main():
    ...
    # ===== ADD-STAGE FEATURE (must match Step4 training) =====
    # stage = r_prev at (t_end - stride) for each sequence
    stage = residual0[prev_idx].astype(np.float32)          # (N_seq,)

    # broadcast to (N_seq, T, 1) so it can be concatenated to X_seq
    stage_seq = np.repeat(stage[:, None, None], repeats=seq_len, axis=1)

    # append to input features: (N_seq, T, 4) -> (N_seq, T, 5)
    X0 = np.concatenate([X0, stage_seq], axis=2).astype(np.float32)
    # ========================================================
    ...
    # ------------------ PATCH: auto-append stage if scaler expects 5 dims ------------------
    D_scaler = int(len(Xm))      # expected feature dim from training
    D_x = int(X.shape[-1])       # current feature dim from NPZ

    print(f"DEBUG: X shape before scaler = {X.shape} | D_x={D_x} | D_scaler={D_scaler}")

    if D_x != D_scaler:
        if D_x == 4 and D_scaler == 5:
            # stage = r_prev = residual0[t_end - stride] = residual0[prev_idx]
            stage = residual0[prev_idx].astype(np.float32)                 # (N_seq,)
            stage_seq = np.repeat(stage[:, None, None], seq_len, axis=1)   # (N_seq, T, 1)
            X = np.concatenate([X, stage_seq], axis=2).astype(np.float32)  # (N_seq, T, 5)
            print(f"✅ Appended stage: X shape = {X.shape}")
        else:
            raise ValueError(f"Feature mismatch: X has D={D_x} but scaler expects D={D_scaler}")

    # now apply scaler safely
    ...
    # ============================================================
    # NEW: one-step hybrid (teacher-forcing state, NO rollout accumulation)
    # r_hat_onestep = r_prev_true + dr_pred
    # ============================================================
    r_prev_test_s = residual0[prev_idx[idx_te]][order].astype(np.float32)  # r(t_end - stride) for each test endpoint
    r_hat_onestep_s = (r_prev_test_s + dr_pred_s).astype(np.float32)
    y_hybrid_onestep = (y_phys_test + r_hat_onestep_s).astype(np.float32)
    # ============================================================
    ...
    metrics = {
        ...
        "Hybrid_onestep": {
            "RMSE": rmse(y_meas_test, y_hybrid_onestep),
            "R2": float(r2_score(y_meas_test, y_hybrid_onestep))
        },
        "Hybrid_last": {
    ...
    plt.plot(y_hybrid_onestep, label="y_hybrid_onestep (r_prev_true + dr_pred)", linewidth=1.2, alpha=0.9)
    plt.title("Test (sorted): y_meas vs y_phys vs y_hybrid_onestep")
    plt.xlabel("Test sample index (sorted endpoints)")
    plt.ylabel("y (log-ratio)")
    plt.legend()
    plt.grid(True, alpha=0.25)
    plt.tight_layout()
    plt.savefig(os.path.join(out_base, "y_compare_test_onestep.png"), dpi=160)
    plt.close()

    plt.figure(figsize=(11, 5))
    plt.plot(r_true_test, label="residual_true", linewidth=1.2)
    plt.plot(r_hat_onestep_s, label="residual_hat_onestep", linewidth=1.2, alpha=0.9)
    plt.title("Test (sorted): residual_true vs residual_hat_onestep")
    plt.xlabel("Test sample index (sorted endpoints)")
    plt.ylabel("residual")
    plt.legend()
    plt.grid(True, alpha=0.25)
    plt.tight_layout()
    plt.savefig(os.path.join(out_base, "residual_compare_test_onestep.png"), dpi=160)
    plt.close()
    ...
    print(f"[Hybrid_onestep] RMSE={metrics['Hybrid_onestep']['RMSE']:.6f} | R2={metrics['Hybrid_onestep']['R2']:.4f}")
    ...
    print("\nSaved outputs to:")
    print(" ", out_base)
    print("  - y_compare_test_onestep.png")
    print("  - residual_compare_test_onestep.png")
